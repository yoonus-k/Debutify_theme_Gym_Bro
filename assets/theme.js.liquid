...
theme.visibilitySettings = (function () {
  ...
  function checkForVisibility(
    ...
    const isCollectionInProductCollections =
    ...
    const lowerProductTags =
      productTags.length &&
      productTags.map((tag) => {
        return tag.toLowerCase();
      });

    const lowerAddOnTags =
      addOnTags &&
      addOnTags
        .split(",")
        .filter((tag) => {
    ...
    const isTagInProductTags =
      lowerProductTags.length &&
      lowerProductTags.some((tag) => {
    ...
    const isTypeInProductType =
      lowerAddOnTypes.length &&
      lowerAddOnTypes.some((type) => {
    ...
    if (templateOptions.visibility === "all") {
      ...
      if (
        templateOptions.visibility === "product" &&
        productId === addOnProductId
      ) {
        addTemplateCallback();
      } else if (
        templateOptions.visibility === "collection" &&
        isCollectionInProductCollections
      ) {
        addTemplateCallback();
...
timber.Drawers = (function () {
  ...
  Drawer.prototype.init = function () {
    var $openBtn = $(
      this.config.open + '[aria-controls="' + this.config.id + '"]'
    );
...
ShopifyAPI.addItemFromForm = function (form, callback, errorCallback) {
  ...
  if (typeof form === "object" && form.data && typeof form.data === "object") {
    data = form.data;
    jsForm =
      form.forms instanceof jQuery
        ? form.forms.get()
        : form.forms instanceof NodeList
        ? Array.from(form.forms)
        : Array(form.forms);
  } else {
    data = $(form).serialize();
    jsForm =
...
theme.ajaxCart = (function (module, $) {
  ...
  init = function (options) {
    ...
    if (
      defaultShopLocale &&
      Shopify.locale === defaultShopLocale.shop_locale.locale
    ) {
  ...
  updateCountPrice = function (cart) {
    ...
    $cartCountSelector.length &&
      $cartCountSelector.forEach((label) => {
        const cartLink = label.parentElement.querySelector(
          settings.bubbleLinkSelector
        );
    ...
    $cartCostSelector.length &&
      $cartCostSelector.forEach((label) => {
        label.innerHTML = `<span class="money">${theme.Currency.formatMoney(
          totalPrice,
  ...
  formSubmitHandler = function (form, evt) {
    ...
    const quantityErrors = document.querySelectorAll(
      `.${settings.quantityErrorClass}`
    );
  ...
  itemErrorCallback = function (XMLHttpRequest, textStatus, form) {
    ...
    if (data.message) {
      if (data.status === 422) {
        ...
        $formToInsert.length &&
          $formToInsert.forEach((form) => {
            const nextElement = form.nextElementSibling;

            if (
              nextElement &&
              !nextElement.classList.contains(settings.quantityErrorClass)
            ) {
  ...
  adjustCart = function () {
    $quantityAdjust = document.querySelectorAll(
      settings.quantityAdjustSelector
    );
    $quantityInput = document.querySelectorAll(settings.quantityInputSelector);
    $quantityRemove = document.querySelectorAll(
      settings.quantityRemoveSelector
    );
    $checkoutSelector = document.querySelectorAll(settings.checkoutSelector);
    $cartNoteSelector = document.querySelectorAll(settings.cartNoteSelector);

    $quantityAdjust.length &&
      $quantityAdjust.forEach((adjustButton) => {
        const quantityHandler = function () {
          if (isUpdating) {
            return;
          }

          const closestProductCard = adjustButton.closest(
            settings.cartProductSelector
          );
          const productCard = closestProductCard
            ? closestProductCard
            : adjustButton.closest(".qty-container");
          const line = productCard.dataset.line;
          const $qtySelector = productCard.querySelector(
    ...
    $quantityInput.length &&
      $quantityInput.forEach((input) => {
        input.addEventListener("change", () => {
          ...
          const closestProductCard = input.closest(
            settings.cartProductSelector
          );
          const productCard = closestProductCard
            ? closestProductCard
            : input.closest(".qty-container");
          const line = productCard.dataset.line;
          const qtyInput = productCard.querySelector(
            settings.quantityInputSelector
          );
          const cartDrawerRow = input.closest(".ajaxcart_row");
          let qty = qtyInput
            ? parseInt(qtyInput.value.replace(/\D/g, ""))
            : null;
    ...
    $quantityRemove.length &&
      $quantityRemove.forEach((removeButton) => {
        removeButton.addEventListener("click", (e) => {
          ...
          const productCard = removeButton.closest(
            settings.cartProductSelector
          );
          const line = productCard.dataset.line;
    ...
    $checkoutSelector.length &&
      $checkoutSelector.forEach((checkoutButton) => {
    ...
    $cartNoteSelector.length &&
      $cartNoteSelector.forEach((cartNote) => {
    ...
    function updateQuantity(line, qty) {
      ...
      const productToUpdate = document.querySelector(
        `${settings.cartProductSelector}[data-line="${line}"]`
      );
  ...
  getCartDrawerMarkup = async function () {
    return theme.fetchTemplate({
      template: isDefaultShopLocaleSet
        ? settings.ajaxCartTemplateName
        : `${Shopify.locale}/${settings.ajaxCartTemplateName}`,
...
theme.NotificationToast = (function () {
  ...
  function init(toastWrapperSelector, callback, isAutoPlayingToastType) {
    ...
    const toastContainer =
      toastWrapper &&
      toastWrapper.querySelector(toastSelectors.toastsContainer);
    ...
    let intervalId;
    const animTime = 1000;
    const intervalTime = toast.dataset.intervalTime
      ? +toast.dataset.intervalTime
      : 10000;
    const displayTime = toast.dataset.displayTime
      ? +toast.dataset.displayTime
      : 5000;
    const totalTime = intervalTime + displayTime + animTime;
    const isCartAjax =
      theme.settings.cartType && theme.settings.cartType !== "page";
    let cartContainer = "";
    let cartOpenClass = "";
    ...
    function initToastMethods() {
      ...
      if (
        isAutoPlayingToastType &&
        sessionStorage.getItem(`toast-${toastWrapper.className}-closed`)
      ) {
        return;
      }
      ...
      toasts &&
        toasts.forEach((toast) => {
          const toastCloseBtn = toast.querySelector(
            toastSelectors.toastCloseBtn
          );
          const cartViewBtn = toast.querySelector(
            toastSelectors.toastRedirectBtn
          );
    ...
    function closeToast(toast, isClosedByButton) {
      ...
      if (isAutoPlayingToastType) {
        ...
        sessionStorage.setItem(
          `toast-${toastWrapper.className}-closed`,
          "true"
        );
    ...
    function redirectOnButtonClick() {
      if (isAutoPlayingToastType) {
        ...
        const mostValuableDiscount = theme.discountCode.getMostValuableDiscount(
          storedDiscounts,
          theme.cart
        );
        ...
        if (mostValuableDiscount) {
          window.location.assign(
            `/checkout?discount=${mostValuableDiscount.name}&locale=${Shopify.locale}`
          );
        } else {
...
theme.ProductQuantities = (function () {
  function setMaxQuantity(quantityContainer, masterVariantSelector, qtyInput) {
    ...
    if (masterVariantSelector) {
      ...
      const currentVariantInventory = currentVariant.hasAttribute(
        "data-remain-qty"
      )
        ? +currentVariant.dataset.remainQty
        : +currentVariant.dataset.productQty;
  ...
  function checkQuantity(quantityContainer, masterVariantSelector, qtyInput) {
    ...
    const variantQty = currentVariant.hasAttribute("data-remain-qty")
      ? +currentVariant.dataset.remainQty
      : +currentVariant.dataset.productQty;
  ...
  async function setVariantQuantities(
    quantityContainer,
    ...
    cart &&
      cart.items.forEach((item) => {
        ...
        const variantOption = masterVariantSelector.querySelector(
          `option[value='${itemId}']`
        );

        if (variantOption) {
          const currentVariantQuantity =
            +variantOption.getAttribute("data-product-qty");

          variantOption.setAttribute(
            "data-remain-qty",
            currentVariantQuantity - itemQuantity
          );
        }
      });
  ...
  function syncQuantityInputs(qtyInput) {
    const productSection = document.querySelector(
      "#mainContent [data-section-type='product-template'] .product-single"
    );
    ...
    const featuredProductSection = qtyInput.closest(
    ...
    const productMainSectionQuantityInput =
      productSection.querySelector(".qty-input");
    const stickyAtcQuantityInput = productPage.querySelector(
...
theme.ProductModel = (function () {
  ...
  function init(modelViewerContainers, sectionId) {
    ...
    setTimeout(() => {
      const modelViewContainer = document.querySelector(
    ...
    function checkIfModelViewIsFullScreen(modelViewContainer) {
      const stackedElements = document.querySelectorAll(
        ".stacked-on-top-of-content"
      );

      if (
        modelViewContainer.classList.contains(
...
theme.modal = (function () {
  ...
  function init() {
    ...
    $(".modal").on("click", function (evt) {
      ...
      if (
        !targetModal.get(0).hasAttribute("data-modal-no-close") &&
        (evt.target === targetModal[0] ||
          evt.target === targetModal.find(".modal-dialog")[0])
      ) {
        close(targetModal);
      }
    });
...
theme.carousel = (function () {
  ...
  function init(options) {
    ...
    if (slider.hasClass(classes.initClass)) {
      var slideIndex = slider
        .find(".slick-current:not(.slick-cloned)")
...
theme.dropdown = function () {
  ...
  document.addEventListener("keyup", (evt) => {
    ...
    var parentDropdowns = getParentsBySelector(
      activeElement,
      selectors.dropdown
    );
    ...
    dropdowns.forEach((dropdown) => {
      const isDropdownInParentDropdowns = parentDropdowns.some(
        (parentDropdown) => {
      ...
      if (
        !isDropdownInParentDropdowns &&
        dropdown.classList.contains(classes.dropdownOpen)
      ) {
        hideDropdown(dropdown);
      }
    });
  ...
  function showDropdown(dropdown) {
    ...
    if (dropdownMenus) {
      dropdownMenus.forEach((dropdownMenu) => {
        ...
        const dropdownViewportMaxHeight =
          window.innerHeight -
          (dropdownViewportOffsetTop + dropdownViewportOffsetBottom);
...
theme.trapFocus = function (options, evt) {
  var tabbable = options.$container.find(
    "select:visible, input:visible, textarea:visible, button:visible, a:visible"
  );
...
theme.discountCode = {
  init: function (passedCart) {
    ...
    let mostValuableDiscount = this.getMostValuableDiscount(
      storedDiscounts,
      theme.cart
    );
    ...
    if (!cart) {
      if (theme.isCartLoaded) {
        mostValuableDiscount = this.getMostValuableDiscount(
          storedDiscounts,
          theme.cart
        );
      } else {
        document.addEventListener("dbtfy:cartLoaded", () => {
          ...
          if (mostValuableDiscount) {
            this.applyDiscount(
              ...
              mostValuableDiscount.type,
              mostValuableDiscount.amount,
              theme.cart
            );
          }
        });
  ...
  getMostValuableDiscount: function (discounts, cart) {
    ...
    const discountsWithFixedAmount = discounts
      .map((discount, index) => {
  ...
  applyDiscount: function (name, type, amount, passedCart) {
    ...
    function initCartForms() {
      cartForms.forEach(function (form) {
        ...
        const cartTotalValueBlock = form.querySelector(
        ...
        const cartTotalFormatted = theme.Currency.formatMoney(
          cartTotal,
          moneyFormat
        );
        const cartTotalWithDiscountFormatted = theme.Currency.formatMoney(
          cartTotalWithDiscount,
          moneyFormat
        );

        if (cartTotalBlock) {
          const discouned = cartTotalBlock.querySelectorAll(
          ...
          if (
            currentTotalValue &&
            currentTotalValue === cartTotalWithDiscount
          ) {
            return;
          } else {
          ...
          const existedTotalValue = cartTotalBlock.querySelector(
            ".cart__subtotal-compare-at"
          );
  ...
  removeDiscount: function (name, options) {
    ...
    const filteredStoredDiscounts = storedDiscounts.filter((discount) => {
      ...
      if (typeof options === "object") {
        isToRemoveByProperties = Object.entries(options).every(
          ([optionName, optionValue]) => {
            return (
              discount.hasOwnProperty(optionName) &&
              discount[optionName] === optionValue
            );
          }
        );
      }
    ...
    sessionStorage.setItem(
      "discounts",
      JSON.stringify(filteredStoredDiscounts)
    );
        ...
        isToRemoveByProperties = Object.entries(options).every(
          ([optionName, optionValue]) => {
            ...
            if (
              ...
              optionValue.checkFunction &&
              optionValue.checkFunction instanceof Function
            ) {
  ...
  destroyDiscount: function (passedCart) {
    ...
    const isDiscount = sessionStorage.getItem("discount")
    ...
    function initCartForms() {
      cartForms.forEach(function (form) {
        const discountFields = form.querySelectorAll("input[name='discount']");
        const cartTotalValueBlocks = document.querySelectorAll(
          "[data-cart-total-value]"
        );
        ...
        if (cartTotal > 0) {
          ...
          if (cartTotalBlocks.length) {
            cartTotalBlocks.forEach(function (block) {
              const existedTotalValue = block.querySelector(
                ".cart__subtotal-compare-at"
              );
...
theme.tabs = (function () {
  ...
  function scroll($tabHeader) {
    ...
    const isStickyAnnouncementBar = document.body.classList.contains(
      "sticky-announcement_bar"
    );
    const announcementBar = document.getElementById("announcement");
    const announcementHeight =
      isStickyAnnouncementBar && announcementBar
    ...
    $("html, body").animate(
      {
        scrollTop:
          $tabHeader.offset().top - headerHeight - announcementHeight - offset,
      },
      theme.variables.animationDuration
    );
...
theme.fetchTemplate = async function (settings, successCallback) {
  let { template, alternativeTemplate } = settings;

  if (!template || !alternativeTemplate) {
    return;
  }

  const templateMarkup = await fetch(
...
theme.isIOSDevice = function () {
  ...
  const isIOSDevice =
    /iP(ad|hone|od)/.test(navigator.userAgent) && !isWindowsPhoneDevice;
    ...
    theme.loadScript(theme.variables.fastClickPluginLink, () => {
      ...
      function disableFastClickForFlashyApp() {
        ...
        function callback() {
          ...
          if (
            flashyContainer &&
            !flashyContainer.classList.contains("needsclick")
          ) {
...
theme.resizeCarousel = function () {
  ...
  theme.cache.$window.on("resize", theme.debounce(refreshSlick, 250));
};
...
theme.showTrialOverPopup = function () {
  ...
  const closeButtons = trialPopup.querySelectorAll(
    ".to-link-btn, [data-toast-close]"
  );
...
theme.syncedProductVariant = function (select, type) {
  ...
  const pickerType = document.querySelector(
  ...
  for (let i = 1; i <= variantOptionsSize; i++) {
    const currentVariantOptionName =
      currentSelectedOption.dataset[`option-${i}`];
    const swatchesOption = singleProductBox.querySelector(
      `.input-color_swatches[value="${currentVariantOptionName}"]`
    );

    if (pickerType === "radio") {
      const fieldsetOptions = document.querySelector(
        `#ProductSelect-option-${i - 1}`
      );

      fieldsetOptions
        .querySelector(
          `.single-option-selector__radio[value="${currentVariantOptionName}"]`
        )
        .click();
    } else {
      const productVariantSelect = document.querySelector(
        `#SingleOptionSelector-${i - 1}`
      );
    ...
    if (swatchesIsPresent && swatchesOption) {
      ...
      let swatchOptionLabel = swatchBox.querySelector(
...
theme.requiredToCheckoutAddons = (function () {
  ...
  function init() {
    ...
    // Delivery date
    if (
      addonsList.dbtfy_delivery_date &&
      theme.settings.dbtfyDeliveryDateRequired
    ) {
    ...
    // Order feedback
    if (
      addonsList.dbtfy_order_feedback &&
      theme.settings.dbtfyOrderFeedbackRequired
    ) {
  ...
  function update(properties) {
    const { name, errors, button, isChanged } = properties;
  ...
  function checkoutButtonsEvents(isChanged) {
    ...
    if (
      amountOfRequiredToCheckoutAddons === amountOfSelectedAddons ||
      isChanged
    ) {
      const additionalCheckoutButtons = document.querySelectorAll(
        ".additional_checkout_buttons"
      );
      const addonsToCheck = isChanged
        ? lastStateOfRequiredAddons
        : selectedAddons;
      ...
      checkoutButtonsToPrevent.forEach((checkoutButtonObject) => {
        const { button, event } = checkoutButtonObject;
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    initProductQuantities: function () {
      let quantityContainer = document.querySelector(
        this.selectors.quantityContainer
      );
      ...
      let masterVariantSelector = document.querySelector(
        this.selectors.masterVariantSelector
      );
      let qtyInput = document.querySelector(this.selectors.qtyInput);
      let productSectionVariantSelectors = document.querySelectorAll(
        ".featured-product-section .single-option-selector__radio, .product-section .single-option-selector__radio"
      );
    ...
    initFullScreenImages: function () {
      ...
      const enableFullScreenImage =
        productSection.data("image-zoom-type") === "image_zoom" ? false : true;
        ...
        afterShow: function () {
          ...
          listItems.forEach((listItem) => {
            ...
            if (!src.includes("cdn.shopify")) {
              ...
              const imageToMatch = document.querySelector(
                `[data-media-href="${src}"]`
              );
            ...
            const badge = document.querySelector(
              `.product-single__thumbnail-badge[data-media-type="${imageType}"]`
            );
    ...
    initProductVariant: function () {
      ...
      $("body").on("ajaxCart.afterCartLoad", function (evt, cart) {
        $($this.selectors.singleOptionSelector, $this.$container).trigger(
          "change"
        );
    ...
    initVariantsChange: function () {
      ...
      $(".single-option-selector__radio", _this.$container).on(
        "change",
        function () {
          const checkedRadioButtons = $(
            ".single-option-selector__radio:checked",
            _this.$container
          );
      ...
      function setOptionLabel(optionSelector) {
        ...
        optionLabel.html(
          `${optionName}: <span class="variant-label-option-value">${optionValue}</span>`
        );
      }
    },
    ...
    showVariantMedia: function (evt) {
      ...
      var $newMedia = $(
        `${this.selectors.productMediaWrapper}[data-media-id="${variantMediaId}"]`
      );
      var $newThumbnail = $(
        `${this.selectors.productThumbnail}[data-media-id="${variantMediaId}"]`
      );
      ...
      if (variant && variant.featured_media) {
        ...
        $(this.selectors.productMediaWrapper, this.$container).removeClass(
          this.classes.featuredMedia
        );

        $newMedia.addClass(this.classes.featuredMedia);
      }
    ...
    createMediaCarousel: function () {
      ...
      var slideIndex = $(
        `${this.selectors.productMediaWrapper}.${this.classes.featuredMedia}`,
        this.$container
      )
      ...
      if (this.isStackedLayout) {
        enquire.register(theme.variables.mediaQuerySmall, {
          match: function () {
            ...
            slideIndex = $(
              `${self.selectors.productMediaWrapper}.${self.classes.featuredMedia}`,
              self.$container
            )
    ...
    destroyMediaCarousel: function () {
      ...
      this.trapCarouselFocus($slider, true);
    ...
    productPage: function (evt) {
      ...
      function initProductPage() {
        const isMainProductSection = this.$container.closest(
          this.selectors.productSection
        ).length;

        if (variant) {
          $(this.selectors.meta, this.$container).removeClass(
            this.classes.unavailable
          );
          ...
          var variantFromSelect = $(
            this.selectors.originalSelectorId,
            this.$container
          ).find(`option[value="${variant.id}"]`);
          var variantInventoryQuantity =
            variantFromSelect.data("inventory-quantity") || 0;
          var variantInventoryPolicy =
            variantFromSelect.data("inventory-policy");
          ...
          if (variant.available) {
            ...
            if (
              ...
              if (
                ...
                $(".dbtfy-quantity_breaks", this.$container)
                  .find(".qb-quantity")
                  .show();
              }
            }
          ...
          function productAvailableToBuy() {
            ...
            $(_this.selectors.addToCartText, _this.$container).html(
              translations.addToCart
            );

            // Remove soldout class
            $(_this.selectors.meta, _this.$container).removeClass(
              _this.classes.soldout
            );
          ...
          function productUnavailableToBuy() {
            ...
            // add soldout class
            $(_this.selectors.meta, _this.$container).addClass(
              _this.classes.soldout
            );
          }
          ...
          $(this.selectors.meta, this.$container).addClass(
            this.classes.unavailable
          );

          if (isMainProductSection) {
            document.body.dispatchEvent(
              new Event("hideStickyAtcOnUnavailableButton")
            );
          }
        }
    ...
    initStickyProductMeta: function () {
      const _this = this;
      const productMeta = $(this.selectors.meta, this.$container).find(
      ...
      function init() {
        var productMediaTemplate = $(
          "[id^=ProductMediaGroup-], [id^=ProductMediaGroup-] .placeholder-svg",
          _this.$container
        );
        var productGeneralInfoTemplate = $(
          _this.selectors.meta,
          _this.$container
        );
        var stickyClass = "large--sticky medium--sticky sticky-check-header";

        if (
          !productMediaTemplate.length ||
          !productGeneralInfoTemplate.length
        ) {
          return;
        }
        ...
        var productGeneralInfoTemplateHeight =
          productGeneralInfoTemplate.height();
        ...
        if (productGeneralInfoTemplateHeight < productMediaTemplateHeight) {
          ...
          productMediaTemplateHeight < productGeneralInfoTemplateHeight
        ) {
...
theme.HeroSlider = (function () {
  function HeroSlider(container) {
    ...
    const transparentHeaderStyles =
      document.querySelector(".hero-header-style");
    ...
    function toggleTransparentHeaderStyles(sectionIndex, onSectionLoad) {
      let $firstSection =
        ...
        document.querySelector(".main-content .shopify-section:first-child");
      ...
      if (sectionIndex === 0 && !onSectionLoad) {
        ...
        sectionIndex > 0 &&
        !onSectionLoad &&
        $firstSection.querySelectorAll("*").length <= 1
      ) {
        $firstSection = $nextSibling && $nextSibling.nextElementSibling;
      }

      const isSectionNotEmpty =
        $firstSection && $firstSection.querySelectorAll("*").length > 1;
      const $nextSection = $firstSection && $firstSection.nextElementSibling;
      const isNextSectionNotEmpty =
        $nextSection && $nextSection.querySelectorAll("*").length > 1;
      ...
      if (
        $firstSection &&
        $firstSection.classList.contains("hero-section") &&
        isSectionNotEmpty
      ) {
        ...
        if (
          $firstSection &&
          !isSectionNotEmpty &&
          $nextSection.classList.contains("hero-section") &&
          isNextSectionNotEmpty
        ) {
          transparentHeaderStyles.setAttribute("media", "all");
        } else {
    ...
    $(document).on("shopify:section:unload", function (event) {
      const sectionIndex = $("#mainContent .shopify-section").index(
        event.target
      );
    ...
    enquire.register(theme.variables.mediaQuerySmall, {
      match: function () {
        ...
        fullHeightSlides.length &&
          fullHeightSlides.each(function () {
            const $slide = $(this);
...
theme.ProductRecommendations = (function () {
  function ProductRecommendations(container) {
    ...
    $.get(recommendationsSectionUrl).then(function (section) {
      const parsedMarkup = new DOMParser().parseFromString(
        section,
        "text/html"
      );
      ...
      const recommendationsMarkup = parsedMarkup.querySelector(
  ...
  window.gm_authFailure = function () {
    ...
    if (Shopify.designMode) {
      ...
      $(selectors.mapOverlay).after(
        `<div class="${classes.errorMsg}">${theme.strings.authError}</div>`
      );
    }
  };
